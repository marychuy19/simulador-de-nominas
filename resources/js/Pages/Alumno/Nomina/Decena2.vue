...
// ✅ RECIBE LOS DATOS QUE VIENEN DESDE DECENA1 (desde el controlador)
...
onMounted(async () => {
  ...
  if (props.prefil3?.empresa) empresa.value = String(props.prefil3.empresa)
  if (props.prefil3?.salario_base != null) salarioBase.value = Number(props.prefil3.salario_base || 0)
  if (props.prefil3?.dias_trabajados != null) diasTrabajados.value = Number(props.prefil3.dias_trabajados || 1)
})

/* =============================
   AL CAMBIAR EMPRESA -> CARGAR EMPLEADOS DECENAL DE ESA EMPRESA
============================= */
watch(empresa, async (empresaId) => {
  ...
  try {
    ...
    if (isPrefil3ing.value && props.prefil3?.empleado) {
      empleado.value = String(props.prefil3.empleado)
    }
...
watch(empleado, (id) => {
  ...
  if (emp) {
    ...
    if (props.prefil3?.salario_base != null && Number(props.prefil3.salario_base) > 0) {
      salarioBase.value = Number(props.prefil3.salario_base)
    } else {
...
const limiteExcentoValesPeriodo = computed(() => {
  return limiteExcentoVales.value / 3
})

const limiteExcentoPermtido = computed(() => Number(props.configNomina?.limite_vales_despensa ?? 0))
const limiteExcentoPermitido10 = computed(() => {
  return limiteExcentoPermtido.value / 3
})
...
const valesGravados = computed(() => {
  ...
  const limite = Number(limiteExcentoPermitido10.value || 0)
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
          ...
          <table class="tabla">
            <tr><td>UMA</td><td>$ {{ uma.toFixed(2) }}</td></tr>
             <tr>
              <td>  Excento decenal</td><td>$ {{ limiteExcentoValesPeriodo.toFixed(2) }}</td>
            </tr>
            <tr>
              <td>Límite excento permitido 40%</td><td>$ {{limiteExcentoPermitido10.toFixed(2) }}</td>
            </tr>
            <tr>
