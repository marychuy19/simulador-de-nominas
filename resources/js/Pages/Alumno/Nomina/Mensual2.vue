...
onMounted(async () => {
  ...
  if (props.prefil4?.empresa) empresa.value = String(props.prefil4.empresa)
  if (props.prefil4?.salario_base != null) salarioBase.value = Number(props.prefil4.salario_base || 0)
  if (props.prefil4?.dias_trabajados != null) diasTrabajados.value = Number(props.prefil4.dias_trabajados || 1)
})
...
watch(empresa, async (empresaId) => {
  ...
  try {
    const resEmpleados = await axios.get('/alumno/empleados-mensuales', {
      params: { empresa_id: empresaId }
    })
    empleados.value = resEmpleados.data

    // ✅ Si venimos de la vista 1, seleccionamos el empleado automáticamente
    if (isPrefil4ing.value && props.prefil4?.empleado) {
      empleado.value = String(props.prefil4.empleado)
    }
...
watch(empleado, (id) => {
  ...
  if (emp) {
    ...
    if (props.prefil4?.salario_base != null && Number(props.prefil4.salario_base) > 0) {
      salarioBase.value = Number(props.prefil4.salario_base)
    } else {
