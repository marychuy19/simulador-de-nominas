...
onMounted(async () => {
  ...
  if (props.prefil1?.empresa) empresa.value = String(props.prefil1.empresa)
  if (props.prefil1?.salario_base != null) salarioBase.value = Number(props.prefil1.salario_base || 0)
  if (props.prefil1?.dias_trabajados != null) diasTrabajados.value = Number(props.prefil1.dias_trabajados || 1)
})
...
const limiteExcentoValesPeriodo = computed(() => {
  return limiteExcentoVales.value / 30.4
})

const limiteExcentoPermtido = computed(() => Number(props.configNomina?.limite_vales_despensa ?? 0))
const limiteExcentoPermitido1 = computed(() => {
  return limiteExcentoPermtido.value / 30.4
})
...
const valesGravados = computed(() => {
  ...
  const limite = Number(limiteExcentoPermitido1.value || 0)
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
          ...
          <table class="tabla">
            <tr><td>UMA</td><td>$ {{ uma.toFixed(2) }}</td></tr>
             <tr>
              <td>  Excento diario</td><td>$ {{ limiteExcentoValesPeriodo.toFixed(2) }}</td>
            </tr>
            <tr>
              <td>Límite excento permitido 40%</td><td>$ {{limiteExcentoPermitido1.toFixed(2) }}</td>
            </tr>
            <tr>
